...
public class AddParameterObjectForPageableArgumentAnnotation extends Recipe {
    ...
    private static final NewAnnotationDescriptor REST_CONTROLLER_ANNOTATION = new NewAnnotationDescriptor("org.springframework.web.bind.annotation",
            "RestController");
    ...
    protected @Nullable TreeVisitor<?, ExecutionContext> getApplicableTest() {
        return new UsesType<>(REST_CONTROLLER_ANNOTATION.fullyQualifiedTypeName());
    }
    ...
    protected TreeVisitor<?, ExecutionContext> getVisitor() {
        return new JavaIsoVisitor<ExecutionContext>() {
            ...
            private boolean hasSuitablePageableArgument(final J.VariableDeclarations p) {
                return p.getLeadingAnnotations().stream().noneMatch(a -> PARAMETER_OBJECT_ANNOTATION.simpleName().equals(a.getSimpleName()))
                        && p.getLeadingAnnotations().stream().anyMatch(a -> PAGEABLE_DEFAULT_ANNOTATION.simpleName().equals(a.getSimpleName()))
                        && Optional.ofNullable(p.getTypeAsFullyQualified()).map(fullyQualified -> fullyQualified.isAssignableTo(PAGEABLE_FULLY_QUALIFIED_NAME))
                                .orElse(false);
            }
        };
    }
